...
import { LobbyRealtimeRefresh } from "@/components/Codenames/LobbyRealtimeRefresh";
import { codenamesLobbyInfo } from "@/components/Codenames/lobbyData";
import type { LobbyRoom } from "@/components/GameLobby/types";

export default async function CodenamesLobbyPage() {
  ...
  const { data: players } = roomIds.length
    ? await supabase.from("codenames_players").select("room_id, name, user_id").in("room_id", roomIds)
    : { data: [] };
  ...
  return (
    <>
      ...
      <GameLobby game={codenamesLobbyInfo} rooms={lobbyRooms} yourRoomCode={yourRoomCode} />
    </>
  );
}
