...
import { LobbyRealtimeRefresh } from "@/components/Koup/LobbyRealtimeRefresh";
import { koupLobbyInfo } from "@/components/Koup/lobbyData";
import type { LobbyRoom } from "@/components/GameLobby/types";

export default async function KoupLobbyPage() {
  ...
  const { data: players } = roomIds.length
    ? await supabase.from("koup_players").select("room_id, name, user_id").in("room_id", roomIds)
    : { data: [] };
  ...
  return (
    <>
      ...
      <GameLobby game={koupLobbyInfo} rooms={lobbyRooms} yourRoomCode={yourRoomCode} />
    </>
  );
}
